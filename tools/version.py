...
def do_main():
    ...
    # version = "v2.40.2-rc1"
    constantFile = os.path.join(env.subst("$BUILD_DIR"), "__compiled_constants.c") # pyright: ignore [reportUndefinedVariable]
    with open(constantFile, "w") as f:
        ...
        sys.stderr.write(f"version.py: BUILD_TIMESTAMP: {timestamp}\n")
        sys.stderr.write(f"version.py: BUILD_BOARD: {env.get('BOARD')}\n") # pyright: ignore [reportUndefinedVariable]

    env.AppendUnique(PIOBUILDFILES=[constantFile]) # pyright: ignore [reportUndefinedVariable]
